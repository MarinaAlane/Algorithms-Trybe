# Função de ordernação quicksort retirada de
# Course da Trybe em Bloco 35 > Algoritmos de Ordenação
# > Conteúdos > Algoritmos Ordenação
...
def is_anagram(first_string, second_string):
    if len(first_string) != len(second_string):
        return False
    first_string_listed = list(first_string)
    second_string_listed = list(second_string)
    quicksort(first_string_listed, 0, len(first_string) - 1)
    quicksort(second_string_listed, 0, len(second_string) - 1)
    return first_string_listed == second_string_listed
